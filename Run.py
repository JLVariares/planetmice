...
class Client(ClientHandler):
        ...
        if this.validatingVersion:
            ...
            if C == 28 and CC == 1:
                ...
                if not ckey == this.server.CKEY:
                    this.server.sendOutput("WARNING: Invalid CKEY ("+ckey+")")
                    this.transport.loseConnection()

                if not validVersion == this.server.Version:
                    this.server.sendOutput("WARNING: Invalid Version ("+validVersion+")")
                    this.transport.loseConnection()
    ...
    def loginPlayer(this, playerName, password, startRoom):	
        this.sendPacket([26, 12], ["http://us3.internet-radio.com:8087/listen.pls&title=PulseEDM%20DANCE%20Music%20Radio&website=http://www.pulseEDM.com"])	
        if this.server.connectedCounts[this.ipAddress] >= 3:
            this.sendPlayerBan(0, "Proibido farm em SeuMice - Ban Permanente.", True)
            this.transport.loseConnection()
        ...
        if len(playerName) < 3 or len(playerName) > 12:
            ...
            this.sendMessage("<br><br><N>«<J> Welcome to MiceFox <N>»<br><br><N>----------------------------------------------------<br><N>«<font color='#33ff33'> Bem Vindos Ao <N>MiceFox<font color='#33ff33'>, Qualquer Duvidas Duvida Pergunte Ao Um Staff, Que Ele Irá Responde-lo</font><N> »<br><N>----------------------------------------------------<br><N>«<font color='#33ff33'> Olá Players,Nosso Mice Está Na Versão <N>1.344<font color='#33ff33'>, Aproveitem.</font><N> »<br><N>----------------------------------------------------<br><N>«<font color='#33ff33'> BR: Digite <N>/AJUDA<font color='#33ff33'>, para abrir o Ajuda</font><N> »<br><N>----------------------------------------------------<br><br>")
    ...
    def updateDatabase(this):
        ...
        if this.privLevel == 0 or this.isGuest:
            this.server.sendOutput("Warning: updating data to priv 0. " + this.Username + ". PlayerID: " + this.playerID + ". Souris: " + this.isGuest)
            return
    ...
    def startPlay(this):
        ...
        if this.room.mapCode == 801:
            this.sendPacket([15, 15], ["-1", "Preck", "4;2,0,2,2,0,0,0,0,1", "680", "340", "0", "0"])
            this.sendPacket([15, 15], ["-2", "Preckaria", "3;10,0,1,0,1,0,0,1,0", "630", "340", "1", "0"]) 
    ...
    def sendPlayerBanLogin(this, hours, reason):
        bantime = 3600000 * hours
        this.sendPacket(Identifiers.old.send.Player_Ban_Login, [bantime, reason])
    ...
    def playerWin(this, holeID, distance=-1):
        if distance != -1 and distance != 1000 and this.isSuspect and this.room.countStats:
            if distance >= 30:
                this.server.sendModMessage(7, "O usuário <V>"+this.Username+"<BL> foi kickado do SeuMice por Atividade Suspeita.")
        ...
        if canGo:
            ...
            if place == 1:
                ...
                this.playerScore += (2 if this.room.isRacing else 16) if not this.room.noAutoScore else 0
                if this.room.getPlayerCountUnique() >= 3:
                    ...
                    if not this.isGuest:
                        this.sendMessage ("<V>[•]<BL> Você ganhou <V>15</V> first e moedas.")
                    if this.room.isRacing or this.isGuest: 
                        this.room.sendAllBin(Identifiers.send.Message, ByteArray().writeUTF("[•] Trocando Mapa em 25 Segundos").toByteArray())
                ...
                if this.room.getPlayerCountUnique() >= 3:
                    ...
                    ##this.iceCoins += 10
                if not this.isGuest:
                    this.sendMessage ("<V>[•]<BL> Você ganhou <V>10</V> first e moedas.")
                ...
                if this.room.getPlayerCountUnique() >= 3:
                   ...
                   # this.iceCoins += 5
                if not this.isGuest:
                    this.sendMessage ("<V>[•]<BL> Você ganhou <V>5</V> first e moedas.")       

                this.playerScore += (3 if this.room.isRacing else 12) if not this.room.noAutoScore else 0
            else:
    ...
    def sendAllModerationChat(this, type, message):
        playerName = this.Username if type == -1 else "" if type == 0 else "Message Serveur" if type == 1 else this.Langue.upper() + "][" + ("Fundador][" if this.privLevel >= 11 else "Admin][" if this.privLevel >= 10 else "Coord][" if this.privLevel == 9 else "Smod][" if this.privLevel == 8 else "Mod][" if this.privLevel == 7 else "MapCrew][" if this.privLevel == 6 else "Helper][" if this.privLevel == 5 else "")
    ...
    def sendStaffLogin(this, isDisconnect):
        playerName = "" + ("Fundador" if this.privLevel >= 11 else "Admin" if this.privLevel >= 10 else "Coord" if this.privLevel == 9 else "SMod" if this.privLevel == 8 else "Mod" if this.privLevel == 7 else "MapCrew" if this.privLevel == 6 else "Helper" if this.privLevel == 5 else "")
...
class Server(protocol.ServerFactory):
    ...
    def updatesuspectList(this):
        with open("./Mice/JSON/SPL.json", "w") as f:
            json.dump(str(this.suspectList), f)
    ...
    def sendPanelRestartMessage(this, seconds):
        if seconds == 120:
            this.sendOutput("[SeuMice] The server will restart in 2 minutes.")
        elif seconds < 120 and seconds > 1:
            this.sendOutput("[SeuMice] The server will restart in "+str(seconds)+" seconds.")
        else:
            this.sendOutput("[SeuMice] The server will restart in 1 second.")
    ...
    def sendEmail(this, code, DestinEmail, playerName):
        ...
        setSubject = "SeuMice"
        setMsg = "Hello "+playerName+", your activation code is: "+str(code)
    ...
    def banPlayer(this, playerName, bantime, reason, modname, silent):
        ...
        if client != None:
            ...
            if not modname == "Server":
                ...
                this.sendModMessage(5, "<V>SeuMice <BL>baniu o jogador <V>"+playerName+"<BL> por <V>1 <BL> hora. Motivo: <V>Vote Populaire<BL>.")
            ...
            if client.privLevel <= 2:
                if not myIP in client.voteBan:
                    ...
                    if len(client.voteBan) >= 10:
                        this.banPlayer(playerName, 1, "Vote Populaire", "SeuMice", False)
    ...
    def sendModChat(this, senderClient, eventTokens, data, binary = None):
        if eventTokens == "\x1A\x04":
            print str(datetime.today())+" [SeuMice] "+data[0]
    ...
    def getPlayersCountMode(this, mode, langue):
        modeName = "SeuMice" if mode == 1 else "SeuMice vanilla" if mode == 3 else "SeuMice survivor" if mode == 8 else "SeuMice racing" if mode == 9 else "SeuMice music" if mode >= 10 else "SeuMice bootcamp" if mode == 2 else "SeuMice defilante" if mode >= 10 else "SeuMice village" if mode == 16 else ""
        playerCount = 0
        for room in this.rooms.values():
    ...
    def checkMessage(this, client, message):
        list = this.suspectList["list"]
        i = 0
        while i < len(list):
            if re.search("[^a-zA-Z]*".join(list[i]), message.lower()):
                this.sendStaffMessage(7, "[<V>Ant-DV</V>] [<T>" + client.roomName + "</T>] [<V>" + client.Username + "</V>] [<N>" + str(message) + "</N>]")
                return True
            i += 1
        return False
    ...
    def setVip(this, playerName, days):
        ...
        if client != None:
            if client.privLevel == 1:
                ...
                client.privLevel = 2
                this.sendModMessage(7, "<V>"+playerName+"<BL> se tornou Vip do SeuMice por <V>"+str(days)+"<BL> dias.")
        else:
    ...
    def checkSuspectBot(this, playerName, type):
        if not this.DEBUG:
            if this.banPlayer(playerName, 360, "Hack Suspect", "SeuMice", False):
                this.sendModMessage(5, "<V>SeuMice <BL>baniu <V>"+playerName+"<BL> por <V>360 <BL>horas. Motivo: <V>Hack Suspect ("+str(type)+")<BL>.")
...
class Room:
    def __init__(this, server, name):
        ...
        elif this.roomName.startswith("#records"):
        ...
        elif this.roomName.startswith("salavip"):
    ...
    def giveSurvivorStats(this):
        for client in this.clients.values():
            if not client.isNewPlayer:
                ...
                if client.survivorStats[0] >= 20 and not str(120) in client.shopBadges:
                    client.shopModule.sendUnlockedBadge(str(120))
                    client.shopBadges.append(str(120))

                if client.survivorStats[1] >= 10 and not str(121) in client.shopBadges:
                    client.shopModule.sendUnlockedBadge(str(121))
                    client.shopBadges.append(str(121))

                if client.survivorStats[2] >= 10 and not str(122) in client.shopBadges:
                    client.shopModule.sendUnlockedBadge(str(122))
                    client.shopBadges.append(str(122))

                if client.survivorStats[3] >= 20 and not str(123) in client.shopBadges:
    ...
    def giveRacingStats(this):
        for client in this.clients.values():
            if not client.isNewPlayer:
                ...
                if client.racingStats[0] >= 10 and not str(124) in client.shopBadges:
                    client.shopModule.sendUnlockedBadge(str(124))
                    client.shopBadges.append(str(124))

                if client.racingStats[1] >= 10 and not str(125) in client.shopBadges:
                    client.shopModule.sendUnlockedBadge(str(125))
                    client.shopBadges.append(str(125))

                if client.racingStats[2] >= 10 and not str(127) in client.shopBadges:
                    client.shopModule.sendUnlockedBadge(str(127))
                    client.shopBadges.append(str(127))

                if client.racingStats[3] >= 10 and not str(126) in client.shopBadges:
...
if __name__ == "__main__":
    ...
    # Connection Server
    os.system("title Transformice 1.344 ")
    os.system('color 7F')
    validPorts = []
    TFM = Server()
    for port in [443, 44440, 44444, 5555, 3724, 6112]:
        try:
            ...
            print "Ports Denied: " + str(port)

    if not validPorts == []:
        print "Roading Ports: " + str(validPorts)
